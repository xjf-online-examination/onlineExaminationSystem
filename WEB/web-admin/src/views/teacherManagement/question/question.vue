<template>
  <div>
    ...
    >
      <Form ref="questionForm" :model="question" :rules="rules">
        ...
        <FormItem label="答案">
          ...
          <Journalizing
            v-if="question.type==='6'&&hackReset"
            type="answer"
    ...
    <Modal v-model="showDeleteModal" :title="'提示'" @on-ok="deleteQuestion">
      <p>是否删除该试题，删除后无法恢复？</p>
    </Modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    deleteQuestion () {
      deleteQuestion(this.tableData.list[this.selectIndex].id).then((res) => {
        ...
        if (valid) {
          ...
          if (this.isAdd) {
            ...
            if (this.question.type === '2') {
              this.question.multipleAnswer = this.multipleAnswer.toString().split(',').join('');
            }
    ...
    handleRowEdit (params) {
      if (params.type === 'add') {
        ...
        this.journalizingData.splice(params.index + 1, 0, obj);
